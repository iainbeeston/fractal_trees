(function() {
  ...
  var branchMean = 8.0;
  var branchStDev = 1.0;
  var branchLength = 7.5;
  var angleMean = Math.PI / 9.0;
  var angleStDev = Math.PI / 18.0;
  ...
  function drawTree(x1, y1, angle1, depth, styles){
    if (depth === 0) return;
    var x2 = x1 + (Math.cos(angle1) * depth * chance.normal({mean: branchMean, dev: branchStDev}));
    var y2 = y1 + (Math.sin(angle1) * depth * chance.normal({mean: branchMean, dev: branchStDev}));
